...
const ModernDashboard = () => {
  ...
  const MiniStatsCard: React.FC<MiniStatsCardProps> = ({ stat }) => {
    const trendData = stat.trend.map((value) => ({ value }));
                ...
                <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700 bg-gray-800' : 'divide-gray-200 bg-white'}`}>
                  {recentActivities.map((activity) => (
                    <tr key={activity.id} className={`${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} transition-colors`}>
                      ...
                      <td className={`px-6 py-4 whitespace-nowrap ${isDarkMode ? 'text-gray-300' : 'text-text-light'}`}>
                        {activity.course}
                      </td>
                      <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-400' : 'text-text-light'}`}>
                        {activity.time}
                      </td>
                      <td className='px-6 py-4 whitespace-nowrap'>
                      ...
                      <td className='px-6 py-4 whitespace-nowrap text-right'>
                        <div className='flex items-center justify-end space-x-2'>
                          <button className={`p-1 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-100 text-gray-600'}`}>
                            <Eye size={16} />
                          </button>
                          <button className={`p-1 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-100 text-gray-600'}`}>
                            <Edit size={16} />
                          </button>
                          <button className={`p-1 rounded-lg transition-colors text-red-500 ${isDarkMode ? 'hover:bg-red-500/10' : 'hover:bg-red-50'}`}>
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
