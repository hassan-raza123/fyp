...
CREATE TABLE `courses` (
    ...
    `type` ENUM('THEORY', 'LAB', 'THEORY_LAB', 'PROJECT', 'THESIS') NOT NULL,
...
CREATE TABLE `courseofferings` (
    ...
    `isResultsLocked` BOOLEAN NOT NULL DEFAULT false,
...
CREATE TABLE `plos` (
    ...
    `bloomDomain` ENUM('Cognitive', 'Affective', 'Psychomotor') NOT NULL DEFAULT 'Cognitive',
    `status` ENUM('active', 'inactive', 'archived') NOT NULL DEFAULT 'active',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
...
CREATE TABLE `llos` (
    ...
    `courseId` INTEGER NOT NULL,
    `bloomLevel` ENUM('Remember', 'Understand', 'Apply', 'Analyze', 'Evaluate', 'Create') NULL,
    `bloomDomain` ENUM('Cognitive', 'Affective', 'Psychomotor') NOT NULL DEFAULT 'Psychomotor',
    `status` ENUM('active', 'inactive', 'archived') NOT NULL DEFAULT 'active',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
...
CREATE TABLE `closattainments` (
    ...
    `unassessedStudents` INTEGER NOT NULL DEFAULT 0,
    `threshold` DOUBLE NOT NULL DEFAULT 50,
...
CREATE TABLE `ploattainments` (
    ...
    `threshold` DOUBLE NOT NULL DEFAULT 50,
    `attainmentPercent` DOUBLE NOT NULL,
    `directAttainment` DOUBLE NULL,
    `indirectAttainment` DOUBLE NULL,
    `isAchieved` BOOLEAN NULL,
    `calculatedAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `calculatedBy` INTEGER NOT NULL,
    `status` ENUM('active', 'archived', 'draft') NOT NULL DEFAULT 'active',
...
CREATE TABLE `assessmentitems` (
    ...
    INDEX `assessmentitems_lloId_idx`(`lloId`),
    INDEX `assessmentitems_rubricId_idx`(`rubricId`),
...
CREATE TABLE `studentgrades` (
    ...
    `studentId` INTEGER NOT NULL,
    `courseOfferingId` INTEGER NOT NULL,
    `component` ENUM('theory', 'lab', 'combined') NOT NULL DEFAULT 'combined',
    `totalMarks` DOUBLE NOT NULL,
    `obtainedMarks` DOUBLE NOT NULL,
    `percentage` DOUBLE NOT NULL,
    `grade` VARCHAR(191) NOT NULL,
    ...
    UNIQUE INDEX `studentgrades_studentId_courseOfferingId_component_attemptNu_key`(`studentId`, `courseOfferingId`, `component`, `attemptNumber`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `obereports` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `reportType` ENUM('clo_attainment', 'llo_attainment', 'plo_attainment', 'program_assessment', 'semester_summary', 'course_wise') NOT NULL,
    `programId` INTEGER NULL,
...
CREATE TABLE `faculty_preferences` (
    ...
    `teachingPreferences` JSON NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    UNIQUE INDEX `faculty_preferences_facultyId_key`(`facultyId`),
    INDEX `faculty_preferences_facultyId_idx`(`facultyId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `student_preferences` (
    ...
    INDEX `student_preferences_studentId_idx`(`studentId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `surveys` (
    ...
    `title` VARCHAR(191) NOT NULL,
    `description` VARCHAR(191) NULL,
    `type` ENUM('course_exit', 'program_exit', 'alumni', 'employer') NOT NULL DEFAULT 'course_exit',
    ...
    UNIQUE INDEX `surveys_publicToken_key`(`publicToken`),
    INDEX `surveys_courseOfferingId_idx`(`courseOfferingId`),
    INDEX `surveys_programId_idx`(`programId`),
    INDEX `surveys_createdBy_idx`(`createdBy`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `survey_questions` (
    ...
    `questionType` ENUM('rating', 'text') NOT NULL DEFAULT 'rating',
    ...
    INDEX `survey_questions_ploId_idx`(`ploId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `survey_responses` (
    ...
    UNIQUE INDEX `survey_responses_surveyId_studentId_key`(`surveyId`, `studentId`),
    UNIQUE INDEX `survey_responses_surveyId_respondentEmail_key`(`surveyId`, `respondentEmail`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `program_curriculum` (
    ...
    `courseCategory` ENUM('core', 'elective', 'lab', 'project', 'thesis') NOT NULL DEFAULT 'core',
    `isRequired` BOOLEAN NOT NULL DEFAULT true,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    INDEX `program_curriculum_programId_idx`(`programId`),
    INDEX `program_curriculum_courseId_idx`(`courseId`),
    UNIQUE INDEX `program_curriculum_programId_courseId_key`(`programId`, `courseId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `action_plans` (
    ...
    `lloId` INTEGER NULL,
    `semesterId` INTEGER NOT NULL,
    `attainmentValue` DOUBLE NOT NULL,
    `threshold` DOUBLE NOT NULL DEFAULT 50,
    `rootCause` TEXT NULL,
    ...
    `followUpAttainmentValue` DOUBLE NULL,
    `isLoopClosed` BOOLEAN NOT NULL DEFAULT false,
    `nextReviewDate` DATETIME(3) NULL,
    `createdBy` INTEGER NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    INDEX `action_plans_courseOfferingId_idx`(`courseOfferingId`),
    INDEX `action_plans_ploId_idx`(`ploId`),
    INDEX `action_plans_cloId_idx`(`cloId`),
    INDEX `action_plans_lloId_idx`(`lloId`),
    INDEX `action_plans_semesterId_idx`(`semesterId`),
    INDEX `action_plans_createdBy_idx`(`createdBy`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `graduation_criteria` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `programId` INTEGER NOT NULL,
    `minCGPA` DOUBLE NOT NULL DEFAULT 2.0,
    `minPloAttainmentPercent` DOUBLE NOT NULL DEFAULT 50,
    `requireAllCourses` BOOLEAN NOT NULL DEFAULT true,
    `directWeight` DOUBLE NOT NULL DEFAULT 0.7,
    `indirectWeight` DOUBLE NOT NULL DEFAULT 0.3,
    `minSurveyResponseRate` DOUBLE NOT NULL DEFAULT 0.0,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    UNIQUE INDEX `graduation_criteria_programId_key`(`programId`),
    INDEX `graduation_criteria_programId_idx`(`programId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `peos` (
    ...
    INDEX `peos_programId_idx`(`programId`),
    UNIQUE INDEX `peos_code_programId_key`(`code`, `programId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `peoplomappings` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `peoId` INTEGER NOT NULL,
    `ploId` INTEGER NOT NULL,
    `weight` DOUBLE NOT NULL DEFAULT 1,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    INDEX `peoplomappings_peoId_idx`(`peoId`),
    INDEX `peoplomappings_ploId_idx`(`ploId`),
    UNIQUE INDEX `peoplomappings_peoId_ploId_key`(`peoId`, `ploId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `rubrics` (
    ...
    INDEX `rubrics_courseOfferingId_idx`(`courseOfferingId`),
    INDEX `rubrics_cloId_idx`(`cloId`),
    INDEX `rubrics_lloId_idx`(`lloId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `rubric_criteria` (
    ...
    `unsatisfactory` TEXT NOT NULL,
    `weight` DOUBLE NOT NULL DEFAULT 1,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    INDEX `rubric_criteria_rubricId_idx`(`rubricId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `_courseprerequisites` (
    ...
    INDEX `_courseprerequisites_B_idx`(`B`),
    PRIMARY KEY (`A`, `B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `_programtocourse` (
    ...
    INDEX `_programtocourse_B_idx`(`B`),
    PRIMARY KEY (`A`, `B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `courseofferings` ADD CONSTRAINT `courseofferings_lockedBy_fkey` FOREIGN KEY (`lockedBy`) REFERENCES `users`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `assessmentitems` ADD CONSTRAINT `assessmentitems_cloId_fkey` FOREIGN KEY (`cloId`) REFERENCES `clos`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `assessmentitems` ADD CONSTRAINT `assessmentitems_lloId_fkey` FOREIGN KEY (`lloId`) REFERENCES `llos`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `assessmentitems` ADD CONSTRAINT `assessmentitems_rubricId_fkey` FOREIGN KEY (`rubricId`) REFERENCES `rubrics`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `faculty_preferences` ADD CONSTRAINT `faculty_preferences_facultyId_fkey` FOREIGN KEY (`facultyId`) REFERENCES `faculties`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `student_preferences` ADD CONSTRAINT `student_preferences_studentId_fkey` FOREIGN KEY (`studentId`) REFERENCES `students`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `surveys` ADD CONSTRAINT `surveys_courseOfferingId_fkey` FOREIGN KEY (`courseOfferingId`) REFERENCES `courseofferings`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `surveys` ADD CONSTRAINT `surveys_programId_fkey` FOREIGN KEY (`programId`) REFERENCES `programs`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `survey_questions` ADD CONSTRAINT `survey_questions_ploId_fkey` FOREIGN KEY (`ploId`) REFERENCES `plos`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `program_curriculum` ADD CONSTRAINT `program_curriculum_programId_fkey` FOREIGN KEY (`programId`) REFERENCES `programs`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `program_curriculum` ADD CONSTRAINT `program_curriculum_courseId_fkey` FOREIGN KEY (`courseId`) REFERENCES `courses`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `action_plans` ADD CONSTRAINT `action_plans_courseOfferingId_fkey` FOREIGN KEY (`courseOfferingId`) REFERENCES `courseofferings`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `action_plans` ADD CONSTRAINT `action_plans_ploId_fkey` FOREIGN KEY (`ploId`) REFERENCES `plos`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `action_plans` ADD CONSTRAINT `action_plans_cloId_fkey` FOREIGN KEY (`cloId`) REFERENCES `clos`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `action_plans` ADD CONSTRAINT `action_plans_lloId_fkey` FOREIGN KEY (`lloId`) REFERENCES `llos`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `action_plans` ADD CONSTRAINT `action_plans_semesterId_fkey` FOREIGN KEY (`semesterId`) REFERENCES `semesters`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `action_plans` ADD CONSTRAINT `action_plans_createdBy_fkey` FOREIGN KEY (`createdBy`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `graduation_criteria` ADD CONSTRAINT `graduation_criteria_programId_fkey` FOREIGN KEY (`programId`) REFERENCES `programs`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `peos` ADD CONSTRAINT `peos_programId_fkey` FOREIGN KEY (`programId`) REFERENCES `programs`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `peoplomappings` ADD CONSTRAINT `peoplomappings_peoId_fkey` FOREIGN KEY (`peoId`) REFERENCES `peos`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `peoplomappings` ADD CONSTRAINT `peoplomappings_ploId_fkey` FOREIGN KEY (`ploId`) REFERENCES `plos`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `rubrics` ADD CONSTRAINT `rubrics_courseOfferingId_fkey` FOREIGN KEY (`courseOfferingId`) REFERENCES `courseofferings`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `rubrics` ADD CONSTRAINT `rubrics_cloId_fkey` FOREIGN KEY (`cloId`) REFERENCES `clos`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `rubrics` ADD CONSTRAINT `rubrics_lloId_fkey` FOREIGN KEY (`lloId`) REFERENCES `llos`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `rubric_scores` ADD CONSTRAINT `rubric_scores_itemResultId_fkey` FOREIGN KEY (`itemResultId`) REFERENCES `studentassessmentitemresults`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `_programtocourse` ADD CONSTRAINT `_programtocourse_A_fkey` FOREIGN KEY (`A`) REFERENCES `programs`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_programtocourse` ADD CONSTRAINT `_programtocourse_B_fkey` FOREIGN KEY (`B`) REFERENCES `courses`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
